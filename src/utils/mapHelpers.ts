 ...
 */
function generateMemoryLocations(): MemoryLocation[] {
  ...
  function generatePointsAroundCenter(center: {lat: number, lng: number, dept: string}, count: number, maxDistanceKm: number) {
    const points = [];
  ...
  Object.entries(regionCoordinates).forEach(([region, centers]) => {
    const regionTotal = regionCounts[region as keyof typeof regionCounts];
    ...
    centers.forEach(center => {
      ...
      points.forEach(point => {
        if (counter >= 140) return; // Limitar a 140 lugares
...
const exileLocations: MemoryLocation[] = [
  {
